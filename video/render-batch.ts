...
const startFrame=parseInt(process.argv[2]||"0");
const endFrame=Math.min(startFrame+1000,TOTAL);
...
for(let f=startFrame;f<endFrame;f++){
  ...
  if(ts<8){
    const t=ts/8;
    if(t<0.2){const fa=t/0.2;glow(W/2,H/2,200,AMBER,0.02*fa);}
  ...
  ctx.fillStyle="rgba(245,158,11,0.1)";ctx.fillRect(0,H-2,W*(f/TOTAL),2);
  const buf=cv.toBuffer("image/jpeg",{quality:0.92});
  fs.writeFileSync(`${FDIR}/f${String(f).padStart(5,"0")}.jpg`,buf);
}
console.log(`Batch done: ${startFrame}-${endFrame-1}`);
