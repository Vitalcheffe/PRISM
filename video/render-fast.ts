// PRISM Launch Film — Optimized for speed
// Uses: single canvas, JPEG output, pre-rendered layers
...
for(let i=0;i<45;i++){const px=bx+rnd()*sBlk,py=by+rnd()*sBlk;
...
function render(frame) {
  ...
  if(ts<8){
    const t=ts/8;
    if(t<0.2){const f=t/0.2;glow(W/2,H/2,200,AMBER,0.02*f);}
  ...
  // Write as JPEG (much faster than PNG)
  const buf=cv.toBuffer("image/jpeg",{quality:0.92});
  fs.writeFileSync(`${FDIR}/f${String(frame).padStart(5,"0")}.jpg`,buf);
}
...
// ── MAIN ──
console.log(`PRISM Launch Film: ${DUR}s @ ${FPS}fps = ${TOTAL} frames (JPEG)`);
genAudio();
const t0=Date.now();
for(let f=0;f<TOTAL;f++){
  render(f);
  if(f%600===0) console.log(`frame ${f}/${TOTAL} · ${((Date.now()-t0)/1000).toFixed(0)}s · ${((f+1)/((Date.now()-t0)/1000)).toFixed(1)} fps`);
}
console.log(`Rendered ${TOTAL} frames in ${((Date.now()-t0)/1000).toFixed(0)}s`);
console.log("Stitching with ffmpeg...");
execSync(`ffmpeg -y -framerate ${FPS} -i ${FDIR}/f%05d.jpg -i ${AUDIO} -c:v libx264 -preset medium -crf 18 -pix_fmt yuv420p -c:a aac -b:a 192k -shortest ${OUT}`,{stdio:"inherit"});
execSync(`rm -f ${FDIR}/f*.jpg`);
