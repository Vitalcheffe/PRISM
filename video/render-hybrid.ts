// render-hybrid.ts — JPEG frames (fast, no memory leak) + ffmpeg upscale to lossless
...
const hystPts=[];for(let i=0;i<=100;i++){const px=W/2-400+(i/100)*800,pr=i/100;let py;
if(pr<0.15)py=H/2+30+100;if(pr<0.4)py=H/2+30+100-(pr-0.15)/0.25*75;else py=H/2+30+25+(pr-0.4)/0.6*75;hystPts.push({x:px,y:py});}
function glow(cx,cy,r,color,a){const g=ctx.createRadialGradient(cx,cy,0,cx,cy,r);g.addColorStop(0,color);g.addColorStop(1,"transparent");
...
function render(frame){const ts=frame/FPS;ctx.drawImage(bgCv,0,0);
...
else{const e=eoc((t-0.5)/0.5);glow(W/2,H/2,400,AMBER,0.05);ctx.strokeStyle=AMBER;ctx.lineWidth=1;ctx.globalAlpha=0.4*(1-e);
ctx.beginPath();ctx.arc(W/2,H/2,8+e*100,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1;ctx.fillStyle=AMBER;ctx.beginPath();ctx.arc(W/2,H/2,5,0,Math.PI*2);ctx.fill();
...
for(let i=0;i<prisms.length;i++){const p=prisms[i];const ap=clamp(t*3-i*0.02,0,1);const h=lerp(0,p.h,eob(ap))+Math.sin(ts*4+i*0.5)*12;
const w=14;ctx.fillStyle=p.color;ctx.globalAlpha=0.5;ctx.fillRect(p.x-w/2,blY-h,w,h);ctx.globalAlpha=1;ctx.fillRect(p.x-w/2,blY-h,w,2);
if(i%7===Math.floor(ts*8)%7)glow(p.x,blY-h,20,p.color,0.12);}ctx.globalAlpha=1;}
else if(ts<50){mono("THE AGENT SWARM",W/2,55,13,FAINT,1);mono("10,000 AGENTS · 8 FACTIONS · REAL REACTIONS",W/2,82,11,FAINT,1);
for(const d of swarm){const ps=d.stress+0.1*Math.sin(ts*5+d.x*0.01);const a=clamp(0.2+ps*0.6,0,1),sz=2+ps*1.5;
ctx.fillStyle=d.color;ctx.globalAlpha=a;ctx.fillRect(d.x,d.y,sz,sz);if(d.stress>0.7)glow(d.x,d.y,6,d.color,0.15);}ctx.globalAlpha=1;
...
ctx.globalAlpha=0.4;ctx.drawImage(grainCv,0,0);ctx.globalAlpha=1;ctx.fillStyle="rgba(245,158,11,0.1)";ctx.fillRect(0,H-2,W*(frame/TOTAL),2);
const buf=cv.toBuffer("image/jpeg",{quality:0.97});fs.writeFileSync(`${FDIR}/f${String(frame).padStart(5,"0")}.jpg`,buf);}
// AUDIO
function genAudio(){const sr=44100,ns=sr*DUR;const buf=new Float32Array(ns*2);
for(let i=0;i<ns;i++){const t=i/sr;let l=0,r=0;const env=clamp(t/2,0,1)*clamp((DUR-t)/3,0,1);
l+=Math.sin(2*Math.PI*55*t)*0.06*env;r+=Math.sin(2*Math.PI*55.3*t)*0.06*env;l+=Math.sin(2*Math.PI*110*t)*0.03*env;r+=Math.sin(2*Math.PI*110.5*t)*0.03*env;
...
const wb=new ArrayBuffer(44+ns*4);const v=new DataView(wb);const ws=(o,s)=>{for(let i=0;i<s.length;i++)v.setUint8(o+i,s.charCodeAt(i));};
ws(0,"RIFF");v.setUint32(4,36+ns*4,true);ws(8,"WAVE");ws(12,"fmt ");v.setUint32(16,16,true);v.setUint16(20,1,true);v.setUint16(22,2,true);
v.setUint32(24,sr,true);v.setUint32(28,sr*4,true);v.setUint16(32,4,true);v.setUint16(34,16,true);ws(36,"data");v.setUint32(40,ns*4,true);
for(let i=0;i<ns*2;i++)v.setInt16(44+i*2,clamp(buf[i],-1,1)*0x7fff,true);fs.writeFileSync(AUDIO,new Uint8Array(wb));console.log("Audio OK");}
fs.mkdirSync(FDIR,{recursive:true});for(const f of fs.readdirSync(FDIR))if(f.endsWith(".jpg"))fs.unlinkSync(`${FDIR}/${f}`);
console.log(`PRISM: ${DUR}s @ ${FPS}fps = ${TOTAL} JPEG frames (q=0.97)`);
genAudio();const t0=Date.now();
for(let f=0;f<TOTAL;f++){render(f);if(f%300===0)console.log(`  ${f}/${TOTAL} · ${((Date.now()-t0)/1000).toFixed(0)}s`);}
console.log(`Rendered in ${((Date.now()-t0)/1000).toFixed(0)}s`);console.log("Stitching...");
execSync(`ffmpeg -y -framerate ${FPS} -i ${FDIR}/f%05d.jpg -i ${AUDIO} -c:v libx264 -preset slow -crf 16 -pix_fmt yuv420p -c:a aac -b:a 192k -shortest ${OUT}`,{stdio:"inherit"});
for(const f of fs.readdirSync(FDIR))if(f.endsWith(".jpg"))fs.unlinkSync(`${FDIR}/${f}`);
