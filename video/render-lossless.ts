// render-lossless.ts — Lossless PNG, no per-frame canvas creation, radial glow = bloom
...
const vig=bgCtx.createRadialGradient(W/2,H/2,0,W/2,H/2,W*0.7);
vig.addColorStop(0,"transparent");vig.addColorStop(1,"rgba(0,0,0,0.4)");
bgCtx.fillStyle=vig; bgCtx.fillRect(0,0,W,H);
const grainCv=createCanvas(W,H); const grainCtx=grainCv.getContext("2d");
...
const st=hot&&rnd()<0.5?0.7+rnd()*0.3:rnd()*0.3;swarm.push({x:px,y:py,color:CAT[cat],stress:st});}}
const waves=[];
...
if(pr<0.15)py=H/2+30+125-25;
else if(pr<0.4)py=H/2+30+125-25-(pr-0.15)/0.25*75;
else py=H/2+30+125-100+(pr-0.4)/0.6*75;
hystPts.push({x:px,y:py});}

function glow(cx,cy,r,color,a){const g=ctx.createRadialGradient(cx,cy,0,cx,cy,r);
g.addColorStop(0,color);g.addColorStop(1,"transparent");
ctx.globalAlpha=a;ctx.fillStyle=g;ctx.fillRect(cx-r,cy-r,r*2,r*2);ctx.globalAlpha=1;}
function mono(t,x,y,s,c,a){ctx.globalAlpha=a??1;ctx.fillStyle=c??SOFT;
ctx.font=`400 ${s}px monospace`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(t,x,y);ctx.globalAlpha=1;}
function big(t,x,y,s,c,w,a){ctx.globalAlpha=a??1;ctx.fillStyle=c??INK;
...
function render(frame){
  ...
  // All effects drawn directly on ctx (glow = bloom substitute)
  if(ts<8){
    const t=ts/8;
    if(t<0.5){const p=0.5+0.5*Math.sin(t*Math.PI*8);
    glow(W/2,H/2,300,AMBER,0.06+p*0.03);
    ctx.fillStyle=AMBER;ctx.beginPath();ctx.arc(W/2,H/2,6+p*4,0,Math.PI*2);ctx.fill();}
    else{const e=eoc((t-0.5)/0.5);
    glow(W/2,H/2,400,AMBER,0.05);
    ...
    const t=(ts-8)/14;
    glow(W/2,H/2,500,AMBER,0.025);
    mono("THE NEURAL NETWORK",W/2,55,13,FAINT,1);
    mono("47 → 32 → 32 → 15 · 3,008 WEIGHTS",W/2,82,11,FAINT,1);
    ...
    ctx.globalAlpha=1;glow(n4.x,n4.y,15,n4.color,0.2);}
    for(const n of nnNodes){const r=n.layer===0?3:n.layer===3?5:4;
    const p=0.5+0.5*Math.sin(ts*3+n.idx*0.1);
    ctx.fillStyle=n.color;ctx.globalAlpha=0.4+p*0.4;ctx.beginPath();ctx.arc(n.x,n.y,r,0,Math.PI*2);ctx.fill();}
    ctx.globalAlpha=1;
    const lbs=["INPUT (47)","HIDDEN-1 (32)","HIDDEN-2 (32)","OUTPUT (15)"];
    for(let i=0;i<4;i++)mono(lbs[i],nnLX[i],H-45,9,FAINT,1);
  } else if(ts<38){
    const t=(ts-22)/16;
    glow(W/2,H/2,500,AMBER,0.025);
    mono("THE REACTOR",W/2,55,13,FAINT,1);
    mono("47 POLICY LEVERS · LIVE PERTURBATION",W/2,82,11,FAINT,1);
    const blY=H*0.8;
    ...
    const w=14;ctx.fillStyle=p.color;ctx.globalAlpha=0.5;ctx.fillRect(p.x-w/2,blY-h,w,h);
    ctx.globalAlpha=1;ctx.fillRect(p.x-w/2,blY-h,w,2);
    if(i%7===Math.floor(ts*8)%7)glow(p.x,blY-h,20,p.color,0.12);}
    ctx.globalAlpha=1;
  } else if(ts<50){
    mono("THE AGENT SWARM",W/2,55,13,FAINT,1);
    mono("10,000 AGENTS · 8 FACTIONS · REAL REACTIONS",W/2,82,11,FAINT,1);
    ...
    if(d.stress>0.7)glow(d.x,d.y,6,d.color,0.15);}
    ctx.globalAlpha=1;
    const lbs=["LABOR","EMPLOYERS","MILITARY","CLERGY","YOUTH","RURAL","URBAN","INFORMAL"];
    for(let r=0;r<2;r++)for(let c=0;c<4;c++)mono(lbs[r*4+c],sX+c*(sBlk+sGap)+sBlk/2,sY+r*(sBlk+sGap)-12,9,CAT[r*4+c],1);
  } else if(ts<62){
    const t=(ts-50)/12;
    glow(W/2,H/2,400,CRIMSON,0.015);
    mono("HYSTERESIS — THE SCAR",W/2,55,13,FAINT,1);
    mono("RECOVERY DOESN'T ERASE THE SCAR",W/2,82,11,FAINT,1);
    ...
    ctx.setLineDash([]);
    mono("CRISIS",crisisX,cy-ph/2-15,9,CRIMSON,1);
    mono("RECOVERY",recX,cy-ph/2-15,9,SOFT,1);
    ...
    ctx.stroke();
    if(dn>60)mono("THE SCAR · +5.0pp",cx+pw*0.15,cy-ph*0.05,11,AMBER,1);
    if(t>0.5)mono("the system remembers",cx,cy+ph/2+35,11,SOFT,clamp((t-0.5)*2,0,1));
  } else {
    const t=(ts-62)/13;
    glow(W/2,H/2,600,AMBER,0.035);
    mono("EMERGENCE",W/2,55,13,FAINT,1);
    ...
    ctx.fillStyle=AMBER;ctx.globalAlpha=0.2+amp*0.6;ctx.fillRect(d.x-1,y-1,2+amp*1.5,2+amp*1.5);}
    ctx.globalAlpha=1;
    if(t>0.3){const a=clamp((t-0.3)/0.2,0,1);
    big("Life is not simulated.",W/2,H/2-80,26,INK,"300",a);
    big("It emerges.",W/2,H/2-40,26,AMBER,"400",a);}
    if(t>0.6){const a=clamp((t-0.6)/0.2,0,1);
    big("PRISM",W/2,H/2+50,44,INK,"700",a);
    mono("github.com/Vitalcheffe/PRISM",W/2,H/2+95,12,SOFT,a);}
  }
...
// AUDIO
function genAudio(){const sr=44100,ns=sr*DUR;const buf=new Float32Array(ns*2);
...
fs.writeFileSync(AUDIO,new Uint8Array(wb));console.log("Audio OK");}
// MAIN
fs.mkdirSync(FDIR,{recursive:true});
for(const f of fs.readdirSync(FDIR))if(f.endsWith(".png"))fs.unlinkSync(`${FDIR}/${f}`);
console.log(`PRISM: ${DUR}s @ ${FPS}fps = ${TOTAL} lossless PNG frames`);
genAudio();
const t0=Date.now();
for(let f=0;f<TOTAL;f++){render(f);if(f%300===0)console.log(`  ${f}/${TOTAL} · ${((Date.now()-t0)/1000).toFixed(0)}s`);}
