// render-final.ts — PRISM Launch Film, lossless PNG, with bloom, 4500 frames
...
// ── Bloom: render to canvas, then blur + composite ──
function bloom(src: any, radius: number): any {
...
const bgCv = createCanvas(W, H);
const bgCtx = bgCv.getContext("2d");
bgCtx.fillStyle = BG; bgCtx.fillRect(0, 0, W, H);
// Dot grid
bgCtx.fillStyle = "rgba(245,158,11,0.025)";
for (let x=0; x<W; x+=40) for (let y=0; y<H; y+=40) bgCtx.fillRect(x, y, 1, 1);
// Vignette
const vigGrad = bgCtx.createRadialGradient(W/2, H/2, 0, W/2, H/2, W*0.7);
...
// ── Grain canvas ──
const grainCv = createCanvas(W, H);
const grainCtx = grainCv.getContext("2d");
const gImg = grainCtx.createImageData(W, H);
for (let i=0; i<gImg.data.length; i+=4) {
  const v = Math.floor(rnd()*255);
  gImg.data[i]=v; gImg.data[i+1]=v; gImg.data[i+2]=v; gImg.data[i+3]=5;
}
...
const nnConns=[];
for (let i=0;i<47;i++) for (let j=0;j<32;j++) if(rnd()<0.1) nnConns.push([i,47+j]);
...
const swarm=[];
const sBlk=180, sGap=50, sC=4, sR=2, sX=(W-(sC*sBlk+(sC-1)*sGap))/2, sY=H*0.22;
for (let r=0; r<sR; r++) for (let c=0; c<sC; c++) {
  ...
  for (let i=0; i<50; i++) { const px=bx+rnd()*sBlk, py=by+rnd()*sBlk;
    const st=hot&&rnd()<0.5?0.7+rnd()*0.3:rnd()*0.3;
    swarm.push({x:px,y:py,color:CAT[cat],stress:st}); }
}

const waves=[];
const wC=60, wR=25, wS=28, wX=(W-wC*wS)/2, wY=(H-wR*wS)/2;
...
function render(frame: number) {
  // Layer 1: background
  const mainCv = cvReuse;
  const ctx = mainCv.getContext("2d");
  ctx.drawImage(bgCv, 0, 0);
  
  const ts = frame / FPS;
  
  // ── EFFECTS LAYER (drawn on separate canvas for bloom) ──
  const fx = fxCvReuse;
  ...
  if (ts < 8) { // HOOK
    const t = ts / 8;
    if (t < 0.5) {
      const p = 0.5+0.5*Math.sin(t*Math.PI*8);
      glow(fctx, W/2, H/2, 300, AMBER, 0.06+p*0.03);
      fctx.fillStyle = AMBER;
      fctx.beginPath(); fctx.arc(W/2, H/2, 6+p*4, 0, Math.PI*2); fctx.fill();
    } else {
      const e = eoc((t-0.5)/0.5);
      glow(fctx, W/2, H/2, 400, AMBER, 0.05);
      fctx.strokeStyle = AMBER; fctx.lineWidth = 1; fctx.globalAlpha = 0.4*(1-e);
      fctx.beginPath(); fctx.arc(W/2, H/2, 8+e*100, 0, Math.PI*2); fctx.stroke(); fctx.globalAlpha = 1;
      fctx.fillStyle = AMBER; fctx.beginPath(); fctx.arc(W/2, H/2, 5, 0, Math.PI*2); fctx.fill();
    }
  } else if (ts < 22) { // NEURAL
    const t = (ts-8)/14;
    glow(fctx, W/2, H/2, 500, AMBER, 0.025);
    fctx.strokeStyle = AMBER; fctx.lineWidth = 0.5;
    for (const [a,b] of nnConns) { const n1=nnNodes[a],n2=nnNodes[b];
      fctx.globalAlpha = 0.02; fctx.beginPath(); fctx.moveTo(n1.x,n1.y); fctx.lineTo(n2.x,n2.y); fctx.stroke(); }
    fctx.globalAlpha = 1;
    for (let s=0; s<4; s++) {
      const si=Math.floor((t*47+s*11)%47), mi=Math.floor((t*32+s*7)%32), oi=Math.floor((t*15+s*3)%15);
      const n1=nnNodes[si],n2=nnNodes[47+mi],n3=nnNodes[79+mi],n4=nnNodes[111+oi];
      fctx.strokeStyle=AMBER; fctx.lineWidth=1.5; fctx.globalAlpha=0.5;
      fctx.beginPath(); fctx.moveTo(n1.x,n1.y); fctx.lineTo(n2.x,n2.y); fctx.lineTo(n3.x,n3.y); fctx.lineTo(n4.x,n4.y); fctx.stroke();
      fctx.globalAlpha=1; glow(fctx, n4.x, n4.y, 15, n4.color, 0.2);
    }
    for (const n of nnNodes) { const r=n.layer===0?3:n.layer===3?5:4;
      const p=0.5+0.5*Math.sin(ts*3+n.idx*0.1);
      fctx.fillStyle=n.color; fctx.globalAlpha=0.4+p*0.4;
      fctx.beginPath(); fctx.arc(n.x,n.y,r,0,Math.PI*2); fctx.fill(); }
    fctx.globalAlpha=1;
  } else if (ts < 38) { // REACTOR
    const t=(ts-22)/16;
    glow(fctx, W/2, H/2, 500, AMBER, 0.025);
    const blY=H*0.8;
    for (let i=0; i<prisms.length; i++) { const p=prisms[i];
      ...
      const w=14; fctx.fillStyle=p.color; fctx.globalAlpha=0.5; fctx.fillRect(p.x-w/2,blY-h,w,h);
      fctx.globalAlpha=1; fctx.fillRect(p.x-w/2,blY-h,w,2);
      if(i%7===Math.floor(ts*8)%7) glow(fctx, p.x, blY-h, 20, p.color, 0.12); }
    fctx.globalAlpha=1;
  } else if (ts < 50) { // SWARM
    for (const d of swarm) { const ps=d.stress+0.1*Math.sin(ts*5+d.x*0.01);
      const a=clamp(0.2+ps*0.6,0,1), sz=2+ps*1.5;
      fctx.fillStyle=d.color; fctx.globalAlpha=a; fctx.fillRect(d.x,d.y,sz,sz);
      if(d.stress>0.7) glow(fctx, d.x, d.y, 6, d.color, 0.15); }
    fctx.globalAlpha=1;
  } else if (ts < 62) { // HYSTERESIS
    const t=(ts-50)/12;
    glow(fctx, W/2, H/2, 400, CRIMSON, 0.015);
    const cx=W/2, cy=H/2+30, pw=800, ph=250;
    const dn=Math.floor(hystPts.length*clamp(t*1.3,0,1));
    if (dn>40) { fctx.fillStyle="rgba(245,158,11,0.06)"; fctx.beginPath();
      for (let i=40; i<dn; i++) i===40?fctx.moveTo(hystPts[i].x,hystPts[i].y):fctx.lineTo(hystPts[i].x,hystPts[i].y);
      for (let i=dn-1; i>=40; i--) fctx.lineTo(hystPts[i].x,hystPts[i].y+50);
      fctx.closePath(); fctx.fill(); }
    fctx.strokeStyle=AMBER; fctx.lineWidth=2; fctx.beginPath();
    for (let i=0; i<dn; i++) { const y=i>40?hystPts[i].y+50:hystPts[i].y;
      i===0?fctx.moveTo(hystPts[i].x,y):fctx.lineTo(hystPts[i].x,y); }
    fctx.stroke();
  } else { // EMERGENCE
    const t=(ts-62)/13;
    glow(fctx, W/2, H/2, 600, AMBER, 0.035);
    for (const d of waves) { const off=Math.sin(d.x*0.005+ts*4)*20+Math.sin(d.x*0.01+ts*5.3)*15+Math.cos(d.y*0.008+ts*3)*10;
      ...
      fctx.fillStyle=AMBER; fctx.globalAlpha=0.2+amp*0.6; fctx.fillRect(d.x-1,y-1,2+amp*1.5,2+amp*1.5); }
    fctx.globalAlpha=1;
  }
  
  // ── BLOOM: blur the FX layer and add it back with screen blend ──
  bloomCtx.clearRect(0,0,W,H); bloomCtx.filter="blur(8px)"; bloomCtx.drawImage(fxCvReuse,0,0); bloomCtx.filter="none"; const bloomCv = bloomCvReuse;
  ...
  if (ts < 8) {
    ...
    for (let r=0; r<2; r++) for (let c=0; c<4; c++) mono(ctx, lbs[r*4+c], sX+c*(sBlk+sGap)+sBlk/2, sY+r*(sBlk+sGap)-12, 9, CAT[r*4+c], 1);
  } else if (ts < 62) {
    const t=(ts-50)/12;
  ...
  // ── Grain ──
  ctx.globalAlpha = 0.4; ctx.drawImage(grainCv, 0, 0); ctx.globalAlpha = 1;
  
  // ── Progress bar ──
  ctx.fillStyle = "rgba(245,158,11,0.1)"; ctx.fillRect(0, H-2, W*(frame/TOTAL), 2);
  
  // Save as lossless PNG
  const buf = mainCv.toBuffer("image/png");
...
console.log(`PRISM Launch Film: ${DUR}s @ ${FPS}fps = ${TOTAL} frames (lossless PNG + bloom)`);
genAudio();
const t0=Date.now();
for (let f=0; f<TOTAL; f++) {
  render(f);
  if (f%300===0) console.log(`  ${f}/${TOTAL} · ${((Date.now()-t0)/1000).toFixed(0)}s`);
}
console.log(`Rendered ${TOTAL} frames in ${((Date.now()-t0)/1000).toFixed(0)}s`);
console.log("Stitching with ffmpeg...");
execSync(`ffmpeg -y -framerate ${FPS} -i ${FDIR}/frame_%05d.png -i ${AUDIO} -c:v libx264 -preset slow -crf 16 -pix_fmt yuv420p -c:a aac -b:a 192k -shortest ${OUT}`, {stdio:"inherit"});
console.log("Cleanup...");
for (const f of fs.readdirSync(FDIR)) if (f.endsWith(".png")) fs.unlinkSync(`${FDIR}/${f}`);
